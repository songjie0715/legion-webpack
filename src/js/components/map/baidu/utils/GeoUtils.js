...
GeoUtils.isPointInRect = function(point, bounds){
    ...
    let sw = bounds.getSouthWest(); //西南脚点
...
GeoUtils.isPointInCircle = function(point, circle){
    ...
    let dis = GeoUtils.getDistance(point, c);
    return dis <= r;
};
...
GeoUtils.isPointOnPolyline = function(point, polyline){
    ...
    let lineBounds = polyline.getBounds();
    ...
    for(let i = 0; i < pts.length - 1; i++){
        ...
        if (point.lng >= Math.min(curPt.lng, nextPt.lng) && point.lng <= Math.max(curPt.lng, nextPt.lng) &&
            ...
            //判断点是否在直线上公式
            let precision = (curPt.lng - point.lng) * (nextPt.lat - point.lat) -
