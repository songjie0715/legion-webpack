...
import h337 from 'heatmap.js/heatmap';

let BMapLib = window.BMapLib = BMapLib || {};

let HeatmapOverlay = BMapLib.HeatmapOverlay = function (opts) {
    this.conf = opts;
...
loader.load('bmap').then(function () {
     ...
     *
     * @example <b>参考示例：</b><br />
     * let map = new BMap.Map("container");<br />map.centerAndZoom(new BMap.Point(116.404, 39.915), 15);<br />let heatmapOverlay = new BMapLib.HeatmapOverlay({"radius":10, "visible":true, "opacity":70});<br />heatmapOverlay.setDataSet(data);//data是热力图的详细数据
     */
    ...
    HeatmapOverlay.prototype.draw = function () {
        ...
        if (this.latlngs.length > 0) {
            ...
            while (len--) {
                let latlng = this.latlngs[len].latlng;
                ...
                let roundedPoint = this.pixelTransform(screenPixel);
                d.data.push({
                    x: roundedPoint.x,
                    y: roundedPoint.y,
    ...
    HeatmapOverlay.prototype.setDataSet = function (data) {
        ...
        while (dlen--) {
            let latlng = new BMap.Point(d[dlen].lng, d[dlen].lat);
            this.latlngs.push({
                latlng: latlng,
                c: d[dlen].count
            });
